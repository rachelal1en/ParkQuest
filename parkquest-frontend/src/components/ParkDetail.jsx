...
export default function ParkDetail() {
  const { id } = useParams();  // ✅ Fix: Ensure correct param name
  ...
  return (
    <div>
      ...
      {park.images?.length > 0 && (
        <img src={park.images[0].url} alt={park.images[0].altText || "Park Image"} width="400" />
      )}
      ...
      {park.operatingHours?.length > 0 && (
        <div>
          ...
          <ul>
            ...
            {/* You can add more logic to handle special hours or seasonal hours if needed */}
          </ul>
        </div>
      )}
    </div>
